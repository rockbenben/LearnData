...
article: false
title: AutoHotkey
description: 想让Windows操作更高效？本文从基础语法到热键设置，详解AutoHotkey核心应用。提供丰富实战脚本样例，助你轻松实现程序速启、弹窗控制等自动化操作，大幅提升工作效率。
icon: fa6-brands:autoprefixer
order: 4
---

AutoHotkey（简称 [AHK](https://www.autohotkey.com/)）是一个运行于 Windows 平台的开源热键脚本语言，主要用于提升操作效率和实现各种自动化功能。其语法简洁，学习门槛较低，适合快速上手。初学者可以借助脚本录制工具开始实践。我个人通过 AHK 实现了热键启动程序、定时判断条件、修改键盘按键映射等自动化操作，推荐学习制作你的专属自动化脚本。

> 本文示例均为 AutoHotkey **v1** 语法；v2 已改为函数式写法、与 v1 差异较大，运行前请注意区分版本。

- [AutoHotkey 中文说明](https://wyagd001.github.io/v1/docs)：[按键 Key 名列表](https://wyagd001.github.io/v1/docs/KeyList.htm)，[AHK 环境变量](https://wyagd001.github.io/v1/docs/Variables.htm#BuiltIn)。
...
| ^    | Ctrl 键                                                                          |
| +    | Shift 键                                                                         |
| `    | 转义字符，参考[转义序列](https://wyagd001.github.io/v1/docs/misc/EscapeChar.htm) |
| ;    | 代表注释内容，不影响函数                                                         |
| ::   | (两个英文冒号)，代表分隔                                                         |

**注意事项**：

- 不区分大小写：如 `run` 和 `RUN` 等价。
- 非热键情况，添加 `return`，脚本全部停止。
- 注意脚本作用域：区分是**全局热键**还是限定在某窗口/程序中。
- 使用管理员权限运行脚本时，所有操作也将以管理员身份执行，即使只是使用普通 `run` 命令。
- AutoHotkey v1 脚本默认使用 ANSI 编码（除非你保存为 **UTF-8-BOM** 编码）.
...
`WinExist` 只能查看前台程序，对于后台或托盘程序，可以用 Process 命令来检查是否存在指定程序的 PID。

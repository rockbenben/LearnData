...
article: false
title: Python
description: Python3实用学习备忘录。涵盖基础语法、常用模块、数据结构、字符串操作，及Lambda函数等进阶用法。无论初学入门还是快速查阅，助你高效掌握核心知识。
...
`sys.path.append("c:\\\\Python37\\\\test")` 添加模块的搜索路径文件夹。`\\` 在 Python 中会转义，所以要输入两个。
...
- step 参数必须是非零整数，否则抛出 ValueError 异常。
...
元组 tuple 与列表 list 用法类似，但元组不能直接修改元素，只能用切片的方法来修改

```python
temp=(1,"xxxxx","迷途",2)      #元组

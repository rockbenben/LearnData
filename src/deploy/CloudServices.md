...
article: false
title: 云计算服务
description: 如何优化网站性能与部署？本篇云计算服务指南，探讨CDN在静态文件托管、图床和全站加速中的应用。同时分享Vercel、Cloudflare Tunnel等第三方服务的部署经验与避坑技巧。
...
静态文件原本使用公开资源库，但国内访问国外静态库容易被墙，访问速度慢；而使用国内静态库有时会遇到服务崩溃，导致无法使用。此外，uBlock 等广告屏蔽扩展会禁用来自公开 CDN 的 js 文件。因此，我选择将静态文件部署到自建 CDN `oss.newzone.top`。
...
- 迁移阿里云 DNS 时，注意关闭**静态协议跟随回源**，否则会导致网站重复调整。
...
# tunnel
sudo docker run --name cloudflaretunnel cloudflare/cloudflared:latest tunnel --no-autoupdate run --token YOUR_TOKEN
...
如果你使用 OpenClash 的 fake-ip，须在「覆写设置」>「规则设置」>「自定义设置」中添加 Cloudflare 的直连规则：

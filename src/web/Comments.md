...
article: false
title: 评论插件
description: 网站评论插件深度指南：对比Waline、Giscus与Gitalk。详解Waline从Vercel部署到Linux配置及数据库迁移全流程，搭建安全高效评论系统。
icon: fa6-solid:comment-dots
order: 3
---

LearnData 使用的是 Waline 评论插件，部署简单，有后端管理。评论插件推荐次序为 Waline > giscus > Gitalk > Gitter。

## 评论插件

- [Waline](https://waline.js.org/guide/get-started/)：基于 Valine 衍生的简洁、安全的评论系统，部署到 Vercel，可匿名互动，导出评论内容，支持文章表情互动。
- [Giscus](https://github.com/giscus/giscus)：基于 github discussion，近似于论坛。LearnData 模板已自带 Giscus，自配置可参考 [如何使用 GitHub 讨论作为你博客的聊天系统](https://blog.csdn.net/duninet/article/details/125280107)。
- [Gitalk](https://github.com/gitalk/gitalk)：基于 github issue，需配置独立库用于存储评论。
- [Gitter](https://gitter.im/)：要登录的公共聊天室。样例如：[boardgame](https://boardgame.io/documentation/#/)，[regauge](https://itchef.github.io/regauge/#/)。
- ~~[Valine](https://github.com/xCss/Valine)：快速、简洁且高效的无后端评论系统，可匿名互动。（最后一版停止在 2022。）~~

## Waline

Waline 支持 Akismet 反垃圾评论和免注册留言，但匿名留言会带来安全隐患。建议开启评论通知来避免出现极端情况。Waline 的评论通知支持多种方式，包括 QQ、微信、邮件等。对于评论的回复，仅支持邮件通知。

Vercel 项目中选择「Overview」>「Settings」>「Environment Variables」，然后按照 [Waline 评论通知](https://waline.js.org/guide/features/notification.html)配置所需环境变量。如果需要 Webhook 等更多通知方式，则查看 [Waline 服务端配置](https://waline.js.org/reference/server/config.html)。配置成功后，选择「Overview」>「Deployments」，进行 Redeploy，重新部署后环境变量和评论通知方会生效。目前 `*.vercel.app` 域名已经被 DNS 污染，会出现「无法加载评论」，国内使用建议绑定自定义域名，域名对备案暂无硬性要求。
...
如果发现系统变量（如 `/etc/environment`）或 Node.js 环境变量未能生效，可改用项目运行路径下的 `.env` 文件加载变量。Node.js 20.6.0 起内置 `--env-file` 参数，无需额外依赖，推荐使用。示例 `package.json` 配置如下：
...
```json
{
  ...
  "version": "0.0.1",
  "scripts": {
    "start": "node --env-file=.env node_modules/@waline/vercel/vanilla.js"
  },
  "dependencies": {
    "@waline/vercel": "^1.40.3"
  }
}
```

该配置通过 Node 原生 `--env-file` 在启动时自动加载 `.env` 文件中的环境变量，确保项目能够正确读取配置。

> 若 Node.js 版本低于 20.6.0（用 `node -v` 查看），则不支持 `--env-file`，需改用 `dotenv-cli`：在 `dependencies` 中加入 `"dotenv-cli": "^11.0.0"`，并将启动脚本写为 `"start": "dotenv node node_modules/@waline/vercel/vanilla.js"`。此时若提示 `dotenv: command not found`，多为未执行 `yarn install` 安装该依赖所致。

若使用企业微信应用通知，请将企业可信 IP 更新为新服务器的 IP 地址。

此外，如果你使用的是 MySQL 8，需注意认证方式的兼容性。Node.js 默认驱动不支持 MySQL 8 的新式认证，需要改回旧版，解决方法参考 [Node.js can't authenticate to MySQL 8.0](https://stackoverflow.com/questions/50373427/node-js-cant-authenticate-to-mysql-8-0)。执行以下命令即可：

```bash
ALTER USER 'waline'@'localhost' IDENTIFIED WITH 'mysql_native_password' BY 'yourpassword';
ALTER USER 'waline'@'127.0.0.1' IDENTIFIED WITH 'mysql_native_password' BY 'yourpassword';
```

### 反向代理

注意：如果你选择手动部署反向代理服务器，请务必使用 Waline 官方提供的 [Nginx 配置文件](https://waline.js.org/guide/deploy/vps.html#nginx-%E9%85%8D%E7%BD%AE)，否则可能会导致**无法登录后台**，或是**版本升级后仍然提示升级**等问题。如果仍然有问题，请检查是否使用 SSH 直连服务器，而非宝塔自带网页终端。宝塔网页终端执行的命令有几率出错，一定要使用正常的 SSH 链接。
...
server
{
  ...
  # SSL setting
  ssl_certificate    /etc/letsencrypt/live/newzone.top/fullchain.pem;
  ssl_certificate_key    /etc/letsencrypt/live/newzone.top/privkey.pem;
...
<script src="https://cdn.staticfile.net/docsify/4.13.1/plugins/gitalk.min.js"></script>
